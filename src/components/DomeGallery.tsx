...
function buildItems(pool: ImageItem[], seg: number) {
  ...
  if (pool.length === 0) {
    return coords.map(c => ({ ...c, src: '', alt: '', title: '', description: '', link: '' }));
  }
...
export const DomeGallery: React.FC<DomeGalleryProps> = ({
  ...
  const lockedRadiusRef = useRef<number>(minRadius);
  ...
  const openItemFromElement = useCallback((el: HTMLElement) => {
    if (openingRef.current || !el.parentElement || !mainRef.current || !frameRef.current || !viewerRef.current) return;
    ...
    // Build Premium Interactive Caption Bar
    const rawTitle = parent.dataset.title || '';
    const rawDesc = parent.dataset.description || '';
    const rawLink = parent.dataset.link || '';

    if (rawTitle || rawDesc) {
      const caption = document.createElement('div');
      caption.className = 'enlarge__caption';
      
      let htmlContent = `<div style="font-family: monospace; font-size: 1.25rem; font-weight: 600; color: #38bdf8; margin-bottom: 0.25rem;">${rawTitle || 'Visual Asset'}</div>`;
      if (rawDesc) {
        htmlContent += `<div style="font-size: 0.85rem; color: rgba(255,255,255,0.8); line-height: 1.4; margin-bottom: 0.75rem; max-height: 3.2rem; overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;">${rawDesc}</div>`;
      }
      if (rawLink && rawLink.trim() !== '' && rawLink.trim() !== '#') {
        htmlContent += `<a href="${rawLink}" target="_blank" rel="noreferrer" style="display: inline-flex; items-center; gap: 0.5rem; padding: 0.4rem 1.25rem; border-radius: 9999px; background: #38bdf8; color: #000; font-family: monospace; font-size: 0.75rem; font-weight: 700; text-decoration: none; align-self: flex-start; cursor: pointer; transition: all 0.2s;" onmouseover="this.style.background='#fff'" onmouseout="this.style.background='#38bdf8'"><span>Explore Enterprise / Go To</span><svg style="width: 12px; height: 12px; display: inline-block;" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg></a>`;
      }
