...
export const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // 2. High-Performance Renderer Setup (Safely Wrapped)
      ...
      try {
        ...
        console.error("Three.js WebGLRenderer creation in PixelBlast failed:", err);
        return;
      }
      ...
      if (noiseAmount > 0) {
        ...
        if (composer && (composer as any).passes.length > 0) {
          (composer as any).passes.forEach((p: any) => (p.renderToScreen = false));
        }
      ...
      renderer.domElement.addEventListener('pointerdown', onPointerDown as any, { passive: true });
      renderer.domElement.addEventListener('pointermove', onPointerMove as any, { passive: true });
      ...
      const io = new IntersectionObserver(
        (entries) => {
          ...
          if (visibilityRef.current.visible && !rafActive) {
            raf = requestAnimationFrame(animate);
            rafActive = true;
          }
        },
      ...
      const t = threeRef.current;
      t.uniforms.uShapeType.value = SHAPE_MAP[variant] ?? 1;
    ...
    return () => {
      ...
      // Force-shrink canvas to 1x1 to free GPU backbuffer memory instantly
      if (t.renderer?.domElement) {
        t.renderer.domElement.width = 1;
        t.renderer.domElement.height = 1;
      }
